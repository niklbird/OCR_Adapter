...
class OPC_Reader():
    ...
    __email__ = "kontakt@vogel-server.de"
    ...
    def __init__(self):
        ...
        try:
            ...
            self.writeError('FATAL ERROR IN CLASS INITIALIZATION: ' + str(e))
            print(e)
            self.opc.close()
    ...
    def screen_0(self):
        ...
        label = self.standardLabel(self.master, 'OPC Tag Reader', 0, 0)
        label.config(font=('Arial', 12, 'bold'))
        b = self.standardButton(self.master, 'Search Server', self.mock, 0, 1)
        b['command'] = lambda listrow=2, listcol=1, label_config=(self.master, '', 0, 2): self.scanOPCServer(listrow,
                                                                                                             listcol,
                                                                                                             label_config)
        mainloop()
    ...
    def screen_1(self):
        ...
        self.standardLabel(self.master, 'Channel.Device.', 0, 4)
        ...
        input_1.focus_set()
        button_1 = self.standardButton(self.master, 'New Tag', self.mock, 2, 5)
        ...
        tmp = self.standardButton(self.master, 'Save', self.mock, 0, 7)
        tmp['command'] = lambda label_config=(self.master, '', 1, 7): self.saveTags(label_config)
        tmp = self.standardButton(self.master, 'Load', self.mock, 0, 8)
        tmp['command'] = lambda channel_input=input_1, label_config=(self.master, '', 1, 7): self.loadTags(
            channel_input, label_config)
        button_2 = self.standardButton(self.master, 'Next', self.screen_2, len(self.serverNames), 8)
    ...
    def screen_2(self):
        ...
        if len(self.labelText_formated) > 1 and not self.labelText_formated[-2] == ',':
            self.labelText_formated = self.labelText_formated[0:-3]
        elif len(self.labelText_formated) > 1:
            self.labelText_formated = self.labelText_formated[0:-2]

        label = self.standardLabel(self.master, 'Test Tags ...', 0, 0)
        self.master.update()
        ...
        label_2 = self.standardLabel(self.master, 'Scan-Interval...', 0, 2)
        self.master.update()
        if (s == ''):
            pass
        else:
            label.config(fg='red', text='! Tags could not be read: ' + s)
            label_2.config(fg='red', text='! ERROR !')
            self.writeError('Screen 2: Tags could not be read: ' + s)
            return -1
        ...
        label_2.config(text='Minimal Scan-Rate: ', font=('Arial', 12, 'bold'))
        tmp_lab3 = self.standardLabel(self.master, '[ ' + str(int(self.scanInterval_limit_ms * 1000)) + ' ms ]', 1, 2)
        tmp_lab3.config(fg='green')
        self.standardLabel(self.master, 'Recording Duration [s]: ', 0, 3)
    ...
    def scanOPCServer(self, listrow, listcol, label_config):
        ...
        try:
            ...
            self.writeError("No Server found")
            return
        ...
        for i in range(0, len(serverlist)):
            ...
            b['command'] = lambda id=serverlist[i], instance=b, label_config=(self.master, '', 0, 3): self.chooseServer(
                id, instance, label_config)
            self.serverNames.append(b)
        if len(serverlist) > 0:
            label = self.standardLabel(label_config[0], 'Server found:', label_config[2], label_config[3])
    ...
    def chooseServer(self, id, instance, label_config):
        ...
        try:
            self.find_in_grid(self.master, label_config[2], label_config[3]).destroy()
        except Exception:
            pass
    ...
    def newTag(self, channel_input, tag_input):
        ...
        button = self.standardButton(self.tagWindow, str(channel_input.get()) + str(tag_input.get()), self.mock, 0,
                                     self.buttonID_Counter)
    ...
    def saveTags(self, label_config):
        ...
        label = self.standardLabel(label_config[0], 'Saved', label_config[2], label_config[3])
    ...
    def loadTags(self, channel_input, label_config):
        ...
        try:
            self.find_in_grid(label_config[0], label_config[2], label_config[3]).destroy()
        except:
            pass
        ...
        try:
            ...
            self.writeError('Load tags: Reading-Error | ' + str(e))
            label = self.standardLabel(label_config[0], 'Reading-Error', label_config[2], label_config[3])
            label.config(fg='red')
        ...
        list = s.split('\n')
        if len(list) < 1:
            self.writeError('Load-Tags: Not enough tags')
            label = self.standardLabel(label_config[0], 'Format-Error', label_config[2], label_config[3])
            label.config(fg='red')
            return -1
        ...
        for i in range(1, len(list) - 1):
            ...
            b = self.standardButton(self.tagWindow,
                                    str(channel_input.get()) + str(list[i]).split(',')[0].replace('"', ''), self.mock,
                                    0, self.buttonID_Counter)
            b['command'] = lambda instance=b: self.removeTag(instance)
            self.addedTags.insert(self.buttonID_Counter, b)
            self.buttonID_Counter += 1

        label = self.standardLabel(label_config[0], 'Read', label_config[2], label_config[3])
        label.config(fg='green')
    ...
    def testTags(self, opc, tagArray):
        ...
        # Remove last comma
        if len(badReads) > 1 and not badReads[-2] == ',':
            badReads = badReads[0:-3] + ' !'
        elif len(badReads) > 1:
            badReads = badReads[0:-2] + ' !'
    ...
    def startRecording(self, duration_lab, scanInterval_lab, button, label_config, button_config, lab_tags, lab_server):
        ...
        if str(duration_lab.get()) == '' or str(scanInterval_lab.get()) == '':
            self.writeError('Start-Recording: Fields are empty')
            print('Empty field')
            return -1

        lab_aktiv = self.standardLabel(label_config[0], 'RECORDING', label_config[2], label_config[3])
        ...
        print('Reading OPC values for ' + str(duration_lab.get()) + ' s with scan-interval: ' + str(
            self.scanInterval_s) + ' s')
        self.master.update()

        dbArray = self.readOPC_values(self.opc, duration_lab.get(), self.scanInterval_s, lab_aktiv, lab_tags,
                                      lab_server)
        self.opc.remove('group')
        if len(dbArray) > 0:
            lab_aktiv.config(text='Recording successful')
            print(len(dbArray))
        else:
            lab_aktiv.config(text='Recording error', fg='red')
            return -1
        ...
        csv = str(csv).replace('.', ',')
        button_2 = self.standardButton(button_config[0], 'Saved', self.mock, button_config[2], button_config[3])
        ...
        self.writeToFile(csv, 'Recording_' + str(now.day) + '.' + str(now.month) + '_' + str(now.hour) +
    ...
    def readOPC_values(self, opc, recordingTime_s, scanInterval_s, lab_aktiv, lab_tags, lab_server):
        ...
        if str(recordingTime_s.split('.')[0]) == 'u':
            permanent = 1
            p.destroy()
            stopButton = self.standardButton(self.master, 'Stop Recording', self.stopRecording, 1, 6)
            stopButton.config(fg='red')
            try:
                writeInterval = int(recordingTime_s.split('.')[1])
            except:
                pass
            try:
            ...
            stopButton = self.standardButton(self.master, 'Stop Recording', self.stopRecording, 0, 6)
            stopButton.config(fg='red')
            try:
                recordingTime_s = int(scanInterval_s) + int(recordingTime_s)
            except:
                return -1
        ...
        while (count < countMax or permanent == 1) and self.stopRec == 0:
            # Loading bar
            if permanent == 0:
                p.config(value=int(count / countMax * 100))
            ...
            # If recording is indefinitely, write out file in writeInterval
                ...
                self.writeToFile(self.CSV_conversion(dataBank, self.labelText.replace(',', ';') + '\n',
                                                     scanInterval_s, self.addedTags),
                                 str(self.folderName) + 'Recording_' + str(now.day) +
    ...
    def standardButton(self, master, textField, com, co, ro):
        ...
        button = Button(master, text=textField, font=('Arial', 12), width=25, height=1, bg="white", borderwidth=4,
